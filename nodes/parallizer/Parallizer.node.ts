...
export class Parallizer implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const parsedRequests = JSON.parse(requests) as { url: string, body: string;method:"GET"|"POST"|"PUT"|"PATCH"|"DELETE" }[];
		const returnItems = [];
		for (const _ of items) {
			const results: INodeExecutionData[] = await Promise.all(parsedRequests.map(async (req) => {
